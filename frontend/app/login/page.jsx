...
function LoginContent() {
  ...
  const passwordHint = form.password && form.password.length < 12
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch('/api/auth/login', {
        ...
        body: JSON.stringify({ ...form, remember: rememberMe }),
      });
            ...
            <div className="mx-auto w-full max-w-md">
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="flex items-center gap-3 py-2">
                  <input
                    id="remember"
                    className="h-5 w-5 rounded border-outline-variant text-primary transition-all focus:ring-primary"
                    type="checkbox"
                    checked={rememberMe}
                    onChange={(event) => setRememberMe(event.target.checked)}
                  />
                  <label className="cursor-pointer text-sm font-medium text-on-surface-variant" htmlFor="remember">
                    Keep me logged in on this device
                  </label>
                </div>

                <button
                  className="editorial-gradient flex w-full items-center justify-center gap-2 rounded-xl py-4 text-sm font-bold text-white shadow-xl shadow-primary/20 transition-all hover:shadow-2xl hover:shadow-primary/30 active:scale-[0.98]"
                  disabled={loading}
                  type="submit"
                >
                  {loading ? 'Accessing...' : 'Access Your Account'}
